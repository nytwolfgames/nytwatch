...
def _parse_test_output(output: str) -> dict[str, bool]:
    ...
    for line in output.splitlines():
        pass_match = re.search(r"Test Completed\.\s+(Auditor\.\S+)\s+Success", line)
        if pass_match:
            results[pass_match.group(1)] = True
            continue

        fail_match = re.search(r"Test Completed\.\s+(Auditor\.\S+)\s+Fail", line)
        if fail_match:
            results[fail_match.group(1)] = False
            continue

        success_match = re.search(r"\[Passed\]\s+(Auditor\.\S+)", line)
        if success_match:
            results[success_match.group(1)] = True
            continue

        failure_match = re.search(r"\[Failed\]\s+(Auditor\.\S+)", line)
        if failure_match:
