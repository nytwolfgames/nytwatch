...
> 48-agent system, 70+ slash commands, and 14 automated hooks. It assumes you
...
After analysis, apply fixes interactively with:

```
/balance-run [system-name]
```
...
Tier 3 (Specialists):  gameplay-programmer, engine-programmer,
                       ...
                       godot-shader-specialist, godot-csharp-specialist,
...
The system has 14 hooks that run automatically:
...
| `notify.sh` | Notification event | Shows desktop notification |
...
| `validate-assets.sh` | After Write/Edit | Checks asset naming and size |
| `validate-skill-change.sh` | Skill file written | Advises running `/skill-test` after `.claude/skills/` changes |
| `queue-code-audit.sh` | After Write/Edit | Queues edited source files for code audit |
...
| `session-stop.sh` | Session end | Queues wiki update; final session logging |
| `code-audit-hook.sh` | Session end | Queues source file changes for `/code-audit` |
...
| Godot C# help | `godot-csharp-specialist` | 3 |
...
| `/balance-run` | Apply balance-check fixes to data files interactively | 6 |
...
| `/code-audit` | Detect source changes and sync GDDs, wiki, ADRs | Any |
...
#### Knowledge Base / Wiki (4)

| Command | Purpose | Phase |
|---------|---------|-------|
| `/code-wiki-init` | Build or rebuild the code/source LLM wiki from scratch | Any |
| `/code-wiki-update` | Apply queued code wiki updates from Stop hook | Any |
| `/narrative-wiki-init` | Build or rebuild the narrative/lore wiki from scratch | Any |
| `/narrative-wiki-update` | Apply answered lore questions to the narrative wiki | Any |
