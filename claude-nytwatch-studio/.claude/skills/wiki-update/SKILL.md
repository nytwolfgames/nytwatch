...
argument-hint: "[optional: specific wiki file or subsystem to update, e.g. subsystems/auth]"
user-invocable: true
...
one design document. Some files merge multiple design documents; some files cover
only part of one. When mapping changed source files to wiki sections, always check
`_index.md` for the correct target — do not assume one-to-one document correspondence.
...
- If an argument was provided (e.g. `/wiki-update subsystems/auth`), treat that as the only target
...
Read any design documents that correspond to the changed area if context is needed.
...
**What to update:**
- New classes, functions, modules, or endpoints → add to the appropriate table or section
...
- A system previously stubbed or incomplete that is now active → update the
...
**Size check:**
- After updating, if a file now exceeds 200 lines, flag it in the Phase 7 summary
...
- If adding a new major sub-concept that will be cross-referenced by 2+ other files,
  add a named anchor: `## Concept Name {#anchor-slug}`
...
If any new class, function, module, or keyword was added in the changed files,
add corresponding rows to `planning/production/wiki/_index.md`.

If a new wiki file was created, add all its major identifiers to the routing table.
