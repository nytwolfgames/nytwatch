...
## Agent Summary
- **Domain**: Rapid throwaway prototypes in the `planning/prototypes/` directory, concept validation experiments, mechanical feasibility tests. Standards intentionally relaxed for speed — prototypes are not production code.
...
- [ ] `description:` field is present and domain-specific (references rapid prototyping, planning/prototypes/ directory, throwaway code)
- [ ] `allowed-tools:` list matches the agent's role (Read/Write scoped to planning/prototypes/ directory; no production src/ write access)
...
**Expected behavior**:
- Produces a minimal working prototype written in the project's engine scripting language, scoped to `planning/prototypes/card-draw-mechanic/`
...
- States clearly: "Prototyper produces throwaway code in planning/prototypes/ to validate concepts; production implementation of validated mechanics is handled by gameplay-programmer"
...
**Expected behavior**:
- Produces a prototype conclusion document in `planning/prototypes/card-draw-mechanic/conclusion.md` (or equivalent)
...
**Expected behavior**:
- Produces a prototype abandonment note in `planning/prototypes/lock-picking-physics/conclusion.md`
...
- Writes the output to `planning/prototypes/grid-movement/` not to `src/`
...
- [ ] Stays within declared domain (planning/prototypes/ directory only; throwaway code for concept validation)
