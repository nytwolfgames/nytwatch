...
**Per-instance tracking toggle** — implement `INytwatchTrackable` on any tracked class to gain a per-instance enable/disable toggle. The default implementation returns `true`; implementing the interface alone does not affect tracking behaviour.
...
The `bEnableNytwatchTrack` property is visible and editable per instance in the editor Details panel. Classes without `INytwatchTrackable` are always tracked (existing behaviour unchanged). Blueprint classes can implement `INytwatchTrackable` and override `IsNytwatchTrackingEnabled` to drive the toggle from a Blueprint variable.
...
4. If the object's class implements `INytwatchTrackable` and `Execute_IsNytwatchTrackingEnabled()` returns `false`, skip the object
5. Call `NytwatchPropertyTracker.PollObject()` for each matching object
6. Append emitted events to `NytwatchSessionWriter` (writer handles flush threshold and cap internally)
...
### 5g. `NytwatchTrackable.h / .cpp`

Responsibility: Opt-in interface for per-instance tracking control.

Key design:
- `UNytwatchTrackable` — the `UInterface` UObject half (required by UHT; no members)
- `INytwatchTrackable` — the native interface class developers inherit from
- Single `BlueprintNativeEvent`: `bool IsNytwatchTrackingEnabled() const` — default implementation returns `true`
- Declared `Blueprintable` so Blueprint classes can implement and override it
- `NytwatchTrackable.cpp` contains only the `UNytwatchTrackable` constructor stub required by UHT

The subsystem checks `UClass::ImplementsInterface(UNytwatchTrackable::StaticClass())` before calling `Execute_IsNytwatchTrackingEnabled()`. The `Execute_` dispatch is generated by UHT and correctly routes to Blueprint or C++ overrides.

Classes that do not implement the interface are unaffected — the check is only entered when `ImplementsInterface` returns true.

### 5h. `NytwatchAgentModule.h / .cpp`
...
| Q10 | Metadata key name | `NytwatchVerbosity` — applied to both `UCLASS` and `UPROPERTY` metadata tags throughout the plugin and all game code annotations. Per-instance enable/disable is handled by `INytwatchTrackable`, not a metadata tag. |
...
| Q14 | Per-instance tracking toggle | `INytwatchTrackable` interface with `IsNytwatchTrackingEnabled()` (default `true`). Checked in the subsystem tick before polling. Classes without the interface are always tracked. Chosen over component injection (Actor-only) and subsystem map (no Details panel) because it works for all `UObject` subclasses, exposes a proper UPROPERTY per instance, and is Blueprint-compatible. |
...
| Per-instance toggle interface | `INytwatchTrackable` |
